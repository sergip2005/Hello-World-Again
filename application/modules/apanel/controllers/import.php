...
class Import extends MY_Controller {
	...
	function _get_saved_import_data($import_id, $page = 0)
	{
		...
		foreach ($return['sheets'] as $n => $sheet) {
			...
			if ($return['sheets'][$n]['data'] != false) {
				...
				$return['sheets'][$n]['prev_state'] = array();
			}

			if (isset($return['post']['model_select']) && $return['post']['model_select'] > 0) {
				$return['current']['model_parts'] = $this->phones_model->getParts($return['post']['vendor_id'], $return['post']['model_select'], 'all', array('not_import' => $import_id));
	...
	public function save(){
		...
		if ($a === 'save_data') {// save provided data

			$model = intval($this->input->post('model_id'));
			$vendor = intval($this->input->post('vendor_id'));
			$sheet = $this->input->post('sheet');

			$n = 0;
			if (!array_key_exists($sheet['type'], $this->sheetTypes) || !array_key_exists('rows', $sheet) || count($sheet['rows']) <= 0) {
				$result['error'] = 'Недоспутимый тип данных листа';
			}

			$model_params = array(
				'import_id' => intval($this->input->post('import_id')),
				'type' => $sheet['type'],
				'vendor_id' => $vendor,
				'phone_id' => $model,
			);
			...
			if (in_array($sheet['type'], array('cabinet', 'solder'))) {// import phone parts data
				foreach ($sheet['rows'] as $rowN => $row_id) {
			...
			// remove saved rows from saved import data
			$this->import_model->remove_sheet_data_from_temp_table($sheet['rows']);

		} elseif ($a === 'remove_unused') {// remove provided parts as unused
